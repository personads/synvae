...
class BamVae(VisualVae):
    def __init__(self, latent_dim, beta, batch_size):
        super().__init__(img_height=64, img_width=64, img_depth=3, latent_dim=latent_dim, beta=beta, batch_size=batch_size, learning_rate=1e-3)
    ...
    def build_decoder(self, latents):
        ...
        dense1 = tf.keras.layers.Dense(units=(self.img_width//16 * self.img_height//16 * 256), activation=tf.nn.relu)(inputs)
        shape = tf.keras.layers.Reshape(target_shape=(self.img_width//16, self.img_height//16, 256))(dense1)
        ...
        deconv4 = tf.keras.layers.Conv2DTranspose(filters=64, kernel_size=3, strides=2, padding='same', activation=tf.nn.relu)(deconv3)
        recons = tf.keras.layers.Conv2DTranspose(filters=self.img_depth, kernel_size=1, strides=1, padding='same', activation=tf.nn.sigmoid)(deconv4)
        return recons
